...
*/
 ...
 SELECT
	...
	cross apply(
        SELECT 
			dbo.fn_GetAplhaNumericOnly(column_name) + ' '
            ...
            + CASE 
                ...
                when data_Type like '%char%' then COALESCE('(' + CASE WHEN character_maximum_length = -1 THEN '16777216' ELSE CAST(character_maximum_length as VARCHAR) END + ')', '')
				else '' 
            END
            + ' ' 
